...
使い方:
  通常生成 (DxDLLW.cs から):
    python gen_hspdxlib.py DxDLLW.cs output_dir [dxlib_help.json]

  リペアモード (DxDLLW.cs を持っていない時、生成済みの .as/.hs を直接修正):
    python gen_hspdxlib.py --repair output_dir
    既存 hspdxlib.hs の %prm セクションから型情報を読み取って、
    hspdxlib.as の `var` パラメータを構造体名 (svalN) に置換し、
    必要な #defstruct ブロックを追加する。
"""
import re
import sys
import os
...
#============================================================
# リペアモード: DxDLLW.cs を持たない状況で生成済み .as を修正
#============================================================

# C 型 → HSP #field 型マップ (.hs ファイルパース用)
_REPAIR_C_TO_HSP_FIELD = {
...
def repair_parse_hs_param_types(hs_path):
    """hspdxlib.hs を読み込み、関数名 → [(型名 or None, パラメータ名), ...] のマップを返す。"""
...
def repair_parse_struct_as_fields(struct_as_path):
    """hspdxlib_struct.as の offset 定数からフィールド情報を復元する。"""
    structs = {}
    ...
    for line in text.split('\n'):
        ...
        if m and cur_struct and m.group(1) == cur_struct:
            ...
            if ftype == 'SIZE':
                continue
            hsp_type = _REPAIR_C_TO_HSP_FIELD.get(ftype)
            if hsp_type:
...
def repair_patch_as_file(as_path, hs_params, structs):
    """既存 hspdxlib.as 内の var パラメータを構造体名に置換し、不足する
    #defstruct ブロックを追加する。返り値は (修正関数数, 追加 defstruct 数)。"""
    ...
    func_pat = re.compile(
        ...
        r'(?:(\w+)\s+)?'                     # cfuncst の戻り値構造体名 (新形式) または None
        r'(\w+)\s+"([^"]+)"\s*(.*?)\s*$'
    )
...
def repair_main(output_dir):
    """--repair モードのエントリポイント。"""
    as_path = os.path.join(output_dir, 'hspdxlib.as')
    hs_path = os.path.join(output_dir, 'hspdxlib.hs')
    struct_as_path = os.path.join(output_dir, 'hspdxlib_struct.as')

    for p in (as_path, hs_path, struct_as_path):
        if not os.path.exists(p):
            print(f'ERROR: {p} not found')
            sys.exit(1)

    print(f'Reading {hs_path}...')
    hs_params = repair_parse_hs_param_types(hs_path)
    print(f'  Parsed {len(hs_params)} function param signatures')

    print(f'Reading {struct_as_path}...')
    structs = repair_parse_struct_as_fields(struct_as_path)
    print(f'  Parsed {len(structs)} struct definitions')

    print(f'Patching {as_path}...')
    fix_count, new_struct_count = repair_patch_as_file(as_path, hs_params, structs)
    print(f'  Fixed {fix_count} function declarations (var → struct name)')
