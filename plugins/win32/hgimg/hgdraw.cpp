...
void hgdraw::NodeToPrim( PNODE *n )
{
	...
	case PRIM_POLY4F:
		...
		a = ((int)(zot * otzv))>>2;			// ���_�̕��ς����
		if ( a <= clipzf ) {
			a += otofs;
			poly->attr = n->attr;
			poly->npoly = n->num >> 2;		// �|���S����
...
//	SetCurrentMatrix( &m_camera );		// T&L���s���̂�
...
{
	if ( pkcurrent == NULL ) {
		pkBeginBuffer();		// �_�u���p�P�b�g�o�b�t�@��������
		if ( bgclr ) {
			io.ClearDest( bgclr,bgcolor,bgtex );
		}
	}
	else {						// OT�`�撆�̓p�P�b�g�ď�����
		pkRestartBuffer();
	}
}
...
void hgdraw::RegistCam( void )
{
	...
	cammoc = MocRegist( cammocnum );		// MOC���m��
	cammode = 0;
...
void hgdraw::ResetLight( void )
{
	...
	lg->flag = 1;
	lg->mode = 0;
	lg->id = MocRegist( LIGHT_MOC_MAX );		// MOC���m��
