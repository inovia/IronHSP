...
def emit_hs(dll_short: str, funcs: List[Func]) -> str:
    ...
    ap("; ============================================================")
    ap("")
    # Header section (fields here become defaults for all records)
    ap("%type")
    ap("拡張命令")
    ap("%ver")
    ap("1.0")
    ...
    ap("2026/04/16")
    ap("%author")
    ap("IronHSP / CsWin32 bridge")
    ap("%url")
    ap("https://github.com/inovia/IronHSP")
    ap("%port")
    ap("Win")
    ap("")
    ap("%note")
    ap(f"Win32 API の {dll_short}.dll 関数群。CsWin32 + win32metadata から自動生成。")
    ap("hsp3net 専用 (intptr / NSTRUCT / wstr を使用)。")
    ap("")
    ap("%group")
    ap("Win32API")
    ap("")
