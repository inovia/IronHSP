...
^
HSP recommends putting wait or await instructions where long loops can occur to pass processing time to other Windows tasks. Running a program that loops for a long time inside an instruction that does not have a wait or await instruction can make it difficult to drag the window or take a long time to move to another task.
...
^p
 ...
 ---------------------------------------------------------------------
  ...
  print | Prints the file in the associated application.
...
^p
Example:
exec "notepad hsp.txt"; open hsp.txt in notepad
^p
...
^p
Example:
exec "hsp.txt", 16; open hsp.txt in the associated application
^p
...
When the end instruction is executed, the application ends.
^
...
Program control instructions
%inst
Specifies the end of the loop specified by the repeat instruction.
...
%inst
Specifies where to jump automatically when the close box (the exit button at the top right of the window) is pressed.
^
...
^p
Normally, pressing the close box will force it to terminate at any time, but after this instruction is executed, pressing the close box will jump to the label specified by the onexit instruction and continue execution from there. It will be like.
^
...
%inst
Terminates a subroutine called by a gosub instruction or a user-defined instruction. The return instruction returns the stack to the caller.
...
The loop instruction returns to the last repeat instruction passed. In other words
^p
	repeat 5
print "Yaho"
	loop
...
^
If you want to force the loop out inside the loop, use the break instruction. There is also a continue instruction to redo the loop.
...
Label 0 to: Label name of the branch destination
%inst
...
In the case of "on numerical goto", the same simple branch as the goto instruction is performed, and in the case of "on numerical gosub", the same subroutine jump as the gosub instruction is performed.
...
While-wend control is achieved using the macro function of the preprocessor.
...
The iterative condition is described by the until macro.
...
^
It is necessary to specify an integer for the initial value and closing value specified by the for macro.
...
See the for macro section for more details.
%href
for
...
The comparison source parameter specified in p1 can be a variable or an expression. After the switch,
^
...
Program control macro
%inst
Iterative macros defined by standard macros,
