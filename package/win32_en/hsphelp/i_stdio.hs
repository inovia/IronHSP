...
%inst
Check the state of the keyboard and mouse buttons and assign them to variables. If the specified button is pressed, 1 is assigned, otherwise 0 is assigned.
...
Specify the coordinates on the display (X = 0 to ginfo_dispx / Y = 0 to ginfo_dispy) instead of the coordinates in the window.
When p1 or p2 is -1, the display of the mouse cursor on the HSP window is turned off.
If you omit the specification of p1 or p2, the current coordinates will be applied as they are.
After turning off the display of the mouse cursor, if you execute only "mouse" that omits p1 and p2, it will be displayed again.

The mouse cursor display settings are controlled by the system's internal counters.
Please note that in order to display the turned off display again, it is necessary to request the display as many times as the number of times it was turned off.
...
^
Random numbers initialized with the same value for p1 will always generate random numbers in the same pattern.
...
^
When the stick command is executed, the following multiple button information is assigned as one numerical value to the variable specified by p1.
^p
...
^
The stick instruction is very similar to the getkey instruction, except that it only detects the moment the button is pressed. In other words, when a button is pressed, it returns the information that was pressed only once, and after that, it is not pressed until the pressed button is released.
^
However, the non-trigger type key specification on p2 makes it possible to detect information even if it is held down.
^
If you specify the code of a key that can be detected even if you hold it down on p2 (the code in the table above. If there are multiple chords, add each number), only that key will be detected as long as the button is pressed. It will be so.
^
This instruction looks very complicated, but it can be a very useful feature when writing scripts with keys.
For example, imagine a shooting game. Your aircraft must move up, down, left and right as long as the button is pressed. However, the next missile will not be fired until the missile launch button is pressed once.
...
(Note that the conditional expression for passing is written in p1.)
^p
...
%inst
Calls the method according to the type of the variable specified by p1.
...
%prm
p1,p2,p3
p1: Minimum value to be output (real value)
p2: Maximum output value (real value)
p3: Formula type value
%inst
...
For example, suppose you want to move an object with an X coordinate of 100 to an X coordinate of 200 with a 50-frame animation.
...
By setting the formula, you can realize organic animation such as starting to move slowly, moving while accelerating, decelerating again in front of the target, moving like a parabola and swinging (shaking). It can be used for various purposes such as movement.
...
As the elapsed value increases, so does the value returned from 100 to 200. And the formula is such that 200 is returned when getease (50,50) is reached.
^p
...
For example, after sorting the array variable a, if the command sortget n, 4 returns a value of 1, the value currently contained in the array variable a.4 will be a.1 before sorting. Indicates that it was in.
