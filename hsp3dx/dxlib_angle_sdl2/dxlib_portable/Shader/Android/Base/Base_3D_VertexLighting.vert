...
void main( void )
{
	...
	// 頂点座標変換 ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++( 終了 )
		...
		vDif  = lDifColor ;
			...
			// 法線をビュー空間の角度に変換( LOCAL_WORLD_MAT はスキニングメッシュ の場合は r0 を使うので mov r0, 0.0 より先に実行する必要がある )
			#ifdef SKINMESH
				lWorldNrm.x = dot( aNrm, lLWMat[ 0 ].xyz ) ;
				lWorldNrm.y = dot( aNrm, lLWMat[ 1 ].xyz ) ;
				lWorldNrm.z = dot( aNrm, lLWMat[ 2 ].xyz ) ;
	...
	// ライトの処理 +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++( 開始 )
			...
			lTotalDif = vec4( 0.0, 0.0, 0.0, 0.0 ) ;
			lTotalSpc = vec4( 0.0, 0.0, 0.0, 0.0 ) ;
	...
	// ディフューズライトとスペキュラライトの角度減衰計算 =======( 開始 )
		...
		lLightLitDest.y = clamp( dot( lViewNrm, -lLightDir ), 0.0, 1.0 ) ;
			...
			// lLightLitDest.z = pow( max( 0.0, N * H ), uMatPower )
			lLightLitDest.z = pow( max( 0.0, dot( lViewNrm, lLightHalfVec ) ), uMatPower ) ;
	...
	// ディフューズカラー計算 =======================================( 開始 )
			...
			lTotalDiffuse.xyz += lLightGen * ( lLightLitDest.y * uLightDif[ 0 ] * lDifColor.xyz + uLightAmb[ 0 ].xyz ) ;

		#else  // LG0_SPOT || LG0_POINT

			// ディフューズライト蓄積値 += ディフューズ角度減衰計算結果 * ディフューズカラー * ライトのディフューズカラー + ライトのアンビエントカラーとマテリアルのアンビエントカラーを乗算したもの
			lTotalDiffuse.xyz += lLightLitDest.y * uLightDif[ 0 ] * lDifColor.xyz + uLightAmb[ 0 ].xyz ;
...
// ライト0の処理 ***********************************************************************************************************************************( 終了 )
	...
	// ディフューズカラー計算 =======================================( 開始 )
			...
			// ディフューズライト蓄積値 += 距離・スポットライト角度減衰値 * ( ディフューズ角度減衰計算結果 * ディフューズカラー * ライトのディフューズカラー + ライトのアンビエントカラーとマテリアルのアンビエントカラーを乗算したもの )
			lTotalDiffuse.xyz += lLightGen * ( lLightLitDest.y * uLightDif[ 1 ] * lDifColor.xyz + uLightAmb[ 1 ].xyz ) ;

		#else  // LG1_SPOT || LG1_POINT

			// ディフューズライト蓄積値 += ディフューズ角度減衰計算結果 * ディフューズカラー * ライトのディフューズカラー + ライトのアンビエントカラーとマテリアルのアンビエントカラーを乗算したもの
			lTotalDiffuse.xyz += lLightLitDest.y * uLightDif[ 1 ] * lDifColor.xyz + uLightAmb[ 1 ].xyz ;
...
// ライト1の処理 ***********************************************************************************************************************************( 終了 )
	...
	// ディフューズカラー計算 =======================================( 開始 )
			...
			// ディフューズライト蓄積値 += 距離・スポットライト角度減衰値 * ( ディフューズ角度減衰計算結果 * ディフューズカラー * ライトのディフューズカラー + ライトのアンビエントカラーとマテリアルのアンビエントカラーを乗算したもの )
			lTotalDiffuse.xyz += lLightGen * ( lLightLitDest.y * uLightDif[ 2 ] * lDifColor.xyz + uLightAmb[ 2 ].xyz ) ;

		#else  // LG2_SPOT || LG2_POINT

			// ディフューズライト蓄積値 += ディフューズ角度減衰計算結果 * ディフューズカラー * ライトのディフューズカラー + ライトのアンビエントカラーとマテリアルのアンビエントカラーを乗算したもの
			lTotalDiffuse.xyz += lLightLitDest.y * uLightDif[ 2 ] * lDifColor.xyz + uLightAmb[ 2 ].xyz ;
