//-----------------------------------------------------------------------------
// 
// 		ＤＸライブラリ		Live2D Cubism4関係プログラム( iOS )
// 
//  	Ver 3.24f
// 
//-----------------------------------------------------------------------------
...
extern BYTE DxShaderCodeBin_Live2D_Cubism4_IOS[] ;
...
{
	Graphics_iOS_ShaderArray_Release( LIVE2DSYS.PF.Shader, LIVE2D_SHADER_Num ) ;
...
extern int Live2DCubism4_Initialize_PF( void )
{
	...
	if( Live2DCubism4_SetupShaderCode( NULL, DxShaderCodeBin_Live2D_Cubism4_IOS ) < 0 )
	{
		return -1 ;
	}
...
extern int Live2DCubism4_GenerateShaders_PF( void )
{
	...
	for( i = 0 ; i < LIVE2D_SHADER_Num ; i ++ )
	{
		...
		Graphics_iOS_Shader_Create( &LIVE2DSYS.PF.Shader[ i ], pVertexShader->PF->Shader, pPixelShader->PF->Shader ) ;
	}
...
extern int Live2DCubism4_SetupShader_PF( int ConstantBufferHandle, D_CubismConstantBufferDxLib *ConstantBuffer, D_ShaderNames VertexShader, D_ShaderNames PixelShader )
{
	...
	GLuint u_matrix        = Graphics_iOS_Shader_GetUniformIndex( &LIVE2DSYS.PF.Shader[ index ], "u_matrix" ) ;
	GLuint u_clipMatrix    = Graphics_iOS_Shader_GetUniformIndex( &LIVE2DSYS.PF.Shader[ index ], "u_clipMatrix" ) ;
	GLuint u_channelFlag   = Graphics_iOS_Shader_GetUniformIndex( &LIVE2DSYS.PF.Shader[ index ], "u_channelFlag" ) ;
	GLuint u_multiplyColor = Graphics_iOS_Shader_GetUniformIndex( &LIVE2DSYS.PF.Shader[ index ], "u_multiplyColor" ) ;
	GLuint u_screenColor   = Graphics_iOS_Shader_GetUniformIndex( &LIVE2DSYS.PF.Shader[ index ], "u_screenColor" ) ;
	GLuint u_baseColor     = Graphics_iOS_Shader_GetUniformIndex( &LIVE2DSYS.PF.Shader[ index ], "u_baseColor" ) ;
	...
	GIOS.Device.State.SetShader_Force = &LIVE2DSYS.PF.Shader[ index ] ;
