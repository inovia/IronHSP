...
// クリップボード関係
extern	int			NS_GetClipboardText(			TCHAR *DestBuffer, int DestBufferBytes DEFAULTPARAM( = -1 ) ) ;									// クリップボードに格納されているテキストデータを読み出す、-1 の場合はクリップボードにテキストデータは無いということ( DestBuffer に NULL を渡すと格納に必要なデータサイズが返ってくる )
...
extern	int			NS_GetSystemInfo_(					int *DxLibVer , int *DirectXVer , int *WindowsVer ) ;	// ＤＸライブラリと DirectX のバージョンと Windows のバージョンを得る
...
extern	int			NS_SetUseWindows10_WM_CHAR_CancelTime(		int Flag ) ;																		// Windows10 で WM_CHAR で短時間に連続して同じ文字が入力された場合の無効扱いにするかどうかを設定する( TRUE:短時間連続入力は無効扱いにする(デフォルト)  FALSE:短時間連続入力も無効にしない )
...
extern	int			NS_GetObtainsStringCharPosition(			int x, int y, int AddY, const TCHAR *String, int StrLen, int *PosX, int *PosY, int FontHandle = -1, int *LineCount = NULL ) ;		// 描画可能領域に収まるように改行しながら文字列を描画した場合の文字列の末端の座標を取得する
extern	int			NS_GetObtainsStringCharPosition_CharClip(	int x, int y, int AddY, const TCHAR *String, int StrLen, int *PosX, int *PosY, int FontHandle = -1, int *LineCount = NULL ) ;		// 描画可能領域に収まるように改行しながら文字列を描画した場合の文字列の末端の座標を取得する( クリップが文字単位 )
extern	int			NS_GetObtainsStringCharPosition_WordClip(	int x, int y, int AddY, const TCHAR *String, int StrLen, int *PosX, int *PosY, int FontHandle = -1, int *LineCount = NULL ) ;		// 描画可能領域に収まるように改行しながら文字列を描画した場合の文字列の末端の座標を取得する( クリップが単語単位 )
#endif // DX_NON_FONT
...
extern	int			NS_DeleteGraph(					int GrHandle ) ;														// 指定のグラフィックハンドルを削除する
...
extern	int			NS_InitGraph(						void ) ;																	// すべてのグラフィックハンドルを削除する
...
extern	int			NS_DrawBox(          int   x1, int   y1, int   x2, int   y2,                                         unsigned int Color, int FillFlag, int   LineThickness = 1    ) ;	// 四角形を描画する
...
extern	int			NS_DrawLineBox(      int   x1, int   y1, int   x2, int   y2,                                         unsigned int Color, int LineThickness = 1 ) ;						// 枠だけの四角形の描画 する
extern	int			NS_DrawCircle(       int   x,  int   y,  int   r,                                                    unsigned int Color, int FillFlag = TRUE, int   LineThickness = 1 ) ;	// 円を描画する
extern	int			NS_DrawCircleAA(     float x,  float y,  float r,            int posnum,                             unsigned int Color, int FillFlag = TRUE, float LineThickness = 1, double Angle = 0.0 ) ;	// 円を描画する( アンチエイリアス付き )
...
extern	int			NS_DrawCylinder3D(  VECTOR   Pos1,   VECTOR   Pos2,      float  r, int DivNum, unsigned int DifColor, unsigned int SpcColor, int FillFlag ) ;		// ３Ｄの円柱を描画する
extern	int			NS_DrawCylinder3DD( VECTOR_D Pos1,   VECTOR_D Pos2,      double r, int DivNum, unsigned int DifColor, unsigned int SpcColor, int FillFlag ) ;		// ３Ｄの円柱を描画する
extern	int			NS_DrawTube3D(      VECTOR   Pos1,   VECTOR   Pos2,      float  r1, float  r2, int DivNum, unsigned int DifColor, unsigned int SpcColor, int FillFlag ) ;		// ３Ｄの筒を描画する
extern	int			NS_DrawTube3DD(     VECTOR_D Pos1,   VECTOR_D Pos2,      double r1, double r2, int DivNum, unsigned int DifColor, unsigned int SpcColor, int FillFlag ) ;		// ３Ｄの筒を描画する
...
extern	int			NS_DrawExtendGraphF(         float x1f, float y1f, float x2f, float y2f,                                               int GrHandle, int TransFlag ) ;							// グラフィックの拡大縮小描画
...
extern	int			NS_SetUseReversedZ(					int Flag ) ;													// Ｚバッファに書き込むＺ値を標準方式と反転した値( リバースＺ )にするかどうかを設定する、DxLib_Init実行前のみ使用可能( TRUE:反転した値にする　FALSE:通常の値にする( デフォルト ) )
...
extern	int			NS_SetVerticalFogMode(					int Mode /* DX_FOGMODE_NONE 等 */ ) ;							// 高さフォグモードを設定する
...
extern	int				NS_GetOtherWindowGraph(				HWND Window, int x1, int y1, int x2, int y2, int GrHandle, int DestX DEFAULTPARAM( = 0 ) , int DestY DEFAULTPARAM( = 0 ) ) ;		// 指定のウィンドウの指定領域の画像情報をグラフィックハンドルに転送する
#endif // WINDOWS_DESKTOP_OS
...
extern	int			NS_SetUseDirect3D11AdapterLUID(					LUID *UseLUID ) ;								// 使用する IDXGIAdapter の LUID を設定する
extern	LUID		NS_GetUseDirect3D11AdapterLUID(					void ) ;										// 使用する IDXGIAdapter の LUID を取得する
...
extern	int			NS_SetUseDirect3D11SwapEffect(					int SwapEffect /* DX_SWAP_EFFECT_DISCARD 等 */ ) ;	// Direct3D11 を使用した場合の SwapEffect を指定する
...
extern	int			NS_UpdateMovieToGraph(					int GraphHandle ) ;															// 動画ファイルの更新処理を行う
...
extern	int			NS_SetUseHalfLambertLighting(	int UseFlag ) ;																	// ３Ｄ描画のライティング計算でハーフランバートを使用するかどうかを設定する( TRUE:ハーフランバートを使用する  FALSE:ハーフランバートを使用しない( デフォルト ) )
...
extern	int			NS_SetFontIgnoreLFFlag(					int Flag ) ;																					// DrawString などで \n を無視するかどうかを設定する( TRUE : 無視する    FALSE : 無視しない( デフォルト ) )
extern	int			NS_GetFontIgnoreLFFlag(					void ) ;																						// DrawString などで \n を無視するかどうかを取得する( TRUE : 無視する    FALSE : 無視しない( デフォルト ) )
...
// サウンドデータ管理系関数
extern	int			NS_InitSoundMem(                        void ) ;																				// 全てのサウンドハンドルを削除する
...
extern	int			NS_DeleteSoundMem(                      int SoundHandle ) ;																		// サウンドハンドルを削除する
