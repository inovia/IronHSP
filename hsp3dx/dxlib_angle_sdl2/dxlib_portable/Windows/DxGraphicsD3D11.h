...
#define D3D11_VERTEX_INPUTLAYOUT_SHADER_2D		(7)				// ( VERTEX2DSHADER                       構造体形式 )シェーダー描画用頂点フォーマット
...
struct GRAPHICS_HARDDATA_DIRECT3D11_DEVICE_STATE
{
	...
	int										VerticalFogEnable ;								// フォグが有効かどうか( TRUE:有効  FALSE:無効 )
	int										VerticalFogMode ;								// フォグモード
	DWORD									VerticalFogColor ;								// フォグカラー
	float									VerticalFogStart ;								// フォグ開始距離
	float									VerticalFogEnd ;								// フォグ終了距離
	float									VerticalFogDensityStart ;						// フォグ密度の開始距離
	float									VerticalFogDensity ;							// フォグ密度
...
//	D_D3D11_TEXTURE2D_DESC			BackBufferTexture2DDesc ;
	...
	int								EnableUserSwapEffect ;					// UserSwapEffect が有効かどうか
	D_DXGI_SWAP_EFFECT				UserSwapEffect ;						// 使用する SwapEffect
...
extern	int		Graphics_D3D11_DeviceState_SetVerticalFogEnable( int Flag ) ;										// 高さフォグを有効にするかどうかを設定する( TRUE:有効  FALSE:無効 )
extern	int		Graphics_D3D11_DeviceState_SetVerticalFogVertexMode( int Mode /* DX_FOGMODE_NONE 等 */ ) ;			// 高さフォグモードを設定する
extern	int		Graphics_D3D11_DeviceState_SetVerticalFogColor( unsigned int Color ) ;								// 高さフォグカラーを変更する
extern	int		Graphics_D3D11_DeviceState_SetVerticalFogStartEnd( float Start, float End ) ;						// 高さフォグが始まる距離と終了する距離を設定する( 0.0f ～ 1.0f )
extern	int		Graphics_D3D11_DeviceState_SetVerticalFogDensity( float start, float Density ) ;					// 高さフォグの密度を設定する( 0.0f ～ 1.0f )
...
extern	int		Graphics_D3D11_DrawSetting_SetLightUseHalfLambert( int UseHalfLambert ) ;						// ライティング計算でハーフランバートを使用するかどうかを設定する
...
extern	int		Graphics_D3D11_DrawLineBox( int x1, int y1, int x2, int y2, unsigned int Color, int Thickness ) ;														// ハードウエアアクセラレータ使用版 DrawLineBox
...
extern	int		Graphics_Hardware_D3D11_SetVerticalFogEnable_PF( int Flag ) ;												// フォグを有効にするかどうかを設定する( TRUE:有効  FALSE:無効 )
extern	int		Graphics_Hardware_D3D11_SetVerticalFogMode_PF( int Mode /* DX_FOGMODE_NONE 等 */ ) ;						// フォグモードを設定する
extern	int		Graphics_Hardware_D3D11_SetVerticalFogColor_PF( DWORD FogColor ) ;											// フォグカラーを変更する
extern	int		Graphics_Hardware_D3D11_SetVerticalFogStartEnd_PF( float start, float end ) ;								// フォグが始まる距離と終了する距離を設定する( 0.0f ～ 1.0f )
extern	int		Graphics_Hardware_D3D11_SetVerticalFogDensity_PF( float start, float density ) ;							// フォグの密度を設定する( 0.0f ～ 1.0f )
...
extern	int			Graphics_Hardware_D3D11_SetUseSwapEffect_PF( D_DXGI_SWAP_EFFECT SwapEffect ) ;								// 使用する SwapEffect を設定する
...
extern	int		Graphics_Hardware_D3D11_Light_SetUseHalfLambert_PF( int UseHalfLambert ) ;										// ライトの計算でハーフランバートを使用するかどうかを設定する
