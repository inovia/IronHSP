...
static DX_DIRECT3D11_RENDER_BLEND_INFO g_DefaultBlendDescArray[ DX_BLENDMODE_NUM ] =
{
	...
	{ RENDER_BLEND_TYPE_NORMAL,			{ TRUE,  D_D3D11_BLEND_ZERO,			D_D3D11_BLEND_ONE,				D_D3D11_BLEND_OP_ADD,			D_D3D11_BLEND_ONE,				D_D3D11_BLEND_ZERO,				D_D3D11_BLEND_OP_ADD,			D_D3D11_COLOR_WRITE_ENABLE_ALL },  FALSE },	// DX_BLENDMODE_DST_RGB_SRC_A		描画元の A のみを書き込む( 描画先の RGB は変更されない )
	{ RENDER_BLEND_TYPE_NORMAL,			{ TRUE,  D_D3D11_BLEND_ZERO,			D_D3D11_BLEND_ONE,				D_D3D11_BLEND_OP_ADD,			D_D3D11_BLEND_INV_DEST_ALPHA,	D_D3D11_BLEND_ZERO,				D_D3D11_BLEND_OP_ADD,			D_D3D11_COLOR_WRITE_ENABLE_ALL },  FALSE },	// DX_BLENDMODE_INVDESTCOLOR_A		描画先の A の反転値を掛ける( 描画先の RGB は変更されない )
	{ RENDER_BLEND_TYPE_NORMAL,			{ TRUE,  D_D3D11_BLEND_ZERO,			D_D3D11_BLEND_ONE,				D_D3D11_BLEND_OP_ADD,			D_D3D11_BLEND_ZERO,				D_D3D11_BLEND_SRC_ALPHA,		D_D3D11_BLEND_OP_ADD,			D_D3D11_COLOR_WRITE_ENABLE_ALL }, FALSE },	// DX_BLENDMODE_MUL_A				A のみの乗算ブレンド( 描画先の RGB は変更されない )
	{ RENDER_BLEND_TYPE_NORMAL,			{ TRUE,  D_D3D11_BLEND_ZERO,			D_D3D11_BLEND_SRC_ALPHA,		D_D3D11_BLEND_OP_ADD,			D_D3D11_BLEND_INV_DEST_ALPHA,	D_D3D11_BLEND_ZERO,				D_D3D11_BLEND_OP_ADD,			D_D3D11_COLOR_WRITE_ENABLE_ALL },  FALSE },	// DX_BLENDMODE_PMA_INVDESTCOLOR_A	乗算済みα用の DX_BLENDMODE_INVDESTCOLOR_A
	{ RENDER_BLEND_TYPE_NORMAL,			{ TRUE,  D_D3D11_BLEND_ZERO,			D_D3D11_BLEND_SRC_ALPHA,		D_D3D11_BLEND_OP_ADD,			D_D3D11_BLEND_ZERO,				D_D3D11_BLEND_SRC_ALPHA,		D_D3D11_BLEND_OP_ADD,			D_D3D11_COLOR_WRITE_ENABLE_ALL }, FALSE },	// DX_BLENDMODE_PMA_MUL_A			乗算済みα用の DX_BLENDMODE_MUL_A
} ;
...
static GRAPHICS_HARDWARE_DIRECT3D11_BASE_SIMPLE_VERTEXSHADER_INFO g_BaseSimpleVertexShaderInfo[ D3D11_VERTEX_INPUTLAYOUT_NUM ] =
{
	...
	// D3D11_VERTEX_INPUTLAYOUT_SHADER_2D( VERTEX2DSHADER )
	{
		"Base_VERTEX2DSHADER.vso",
		sizeof( VERTEX_BLENDTEX_2D ),
		{
			{ "POSITION",     0, D_DXGI_FORMAT_R32G32B32A32_FLOAT, 0, 0,  D_D3D11_INPUT_PER_VERTEX_DATA, 0 },
			{ "COLOR",        0, D_DXGI_FORMAT_R8G8B8A8_UNORM,     0, 16, D_D3D11_INPUT_PER_VERTEX_DATA, 0 },
...
static int Graphics_D3D11_DeviceState_UpdateConstantVerticalFogParam( void ) ;						// 高さフォグの色以外の定数情報を更新する
...
static int CreateD3D11BaseSimpleVertexShader( int InputLayout )
{
	...
	if( InputLayout == D3D11_VERTEX_INPUTLAYOUT_SHADER_2D )
	{
		Macro[ MacroNum ].Name = NULL ;
		Macro[ MacroNum ].Definition = NULL ;
		MacroNum ++ ;

		return Graphics_D3D11_CompileVertexShader( "Base_2D_VS.hlsl", "VS_Shader2D", Macro, &GD3D11.Device.Shader.Base.BaseSimple_VS[ InputLayout ], &GD3D11.ShaderCode.Base.BaseSimple_VS_Blob[ InputLayout ] ) ;
	}
	else
...
static	int		Graphics_D3D11_OutputWindow_CreateSwapChain( int Index )
{
	...
	if( GAPIWin.DXGIFactory2Object != NULL )
	{
		...
		// スワップチェインの作成
		{
			...
			SwapChainDesc1.SwapEffect				= GD3D11.Device.Screen.EnableUserSwapEffect ? GD3D11.Device.Screen.UserSwapEffect : D_DXGI_SWAP_EFFECT_DISCARD ;
	...
	else
	{
		...
		// スワップチェインの作成
		{
			...
			SwapChainDesc.SwapEffect							= GD3D11.Device.Screen.EnableUserSwapEffect ? GD3D11.Device.Screen.UserSwapEffect : D_DXGI_SWAP_EFFECT_DISCARD ;
...
extern	int		Graphics_D3D11_OutputWindow_Add( HWND TargetWindow, int IsMainTarget )
{
	...
	// 無効になっているウィンドウがあったら出力先ウィンドウから外す
	OWI = GD3D11.Device.Screen.OutputWindowInfo ;
...
extern	int		Graphics_D3D11_Device_Initialize( void )
{
	...
	// 描画ステータスのセット
	{
		...
		Graphics_D3D11_DeviceState_SetVerticalFogEnable( FALSE ) ;
		Graphics_D3D11_DeviceState_SetVerticalFogColor( 0 ) ;
		Graphics_D3D11_DeviceState_SetVerticalFogVertexMode( DX_FOGMODE_NONE ) ;
...
extern	int		Graphics_D3D11_Device_ReInitialize( void )
{
	...
	// 削除前と同じ環境を作り出す
	{
		...
		Graphics_D3D11_DeviceState_SetVerticalFogEnable(     GD3D11.Device.State.VerticalFogEnable ) ;
		Graphics_D3D11_DeviceState_SetVerticalFogVertexMode( GD3D11.Device.State.VerticalFogMode ) ;
		Graphics_D3D11_DeviceState_SetVerticalFogColor(      GD3D11.Device.State.VerticalFogColor ) ;
...
static int Graphics_D3D11_DeviceState_UpdateConstantFogParam( void )
{
	...
	ConstantCommon->Fog.DensityStart = 0.0f ;
...
extern int  Graphics_D3D11_DeviceState_SetVerticalFogEnable( int Flag )
{
	...
	if( Flag == GD3D11.Device.State.VerticalFogEnable &&
		GD3D11.Device.DrawSetting.CancelSettingEqualCheck == FALSE )
	{
		return 0 ;
	}
	...
	GD3D11.Device.State.VerticalFogEnable = Flag ;
...
// 高さフォグモードを設定する
extern int  Graphics_D3D11_DeviceState_SetVerticalFogVertexMode( int Mode /* DX_FOGMODE_NONE 等 */ )
{
	if( GAPIWin.D3D11DeviceObject == NULL )
	{
		return -1 ;
	...
	if( Mode == GD3D11.Device.State.VerticalFogMode &&
		GD3D11.Device.DrawSetting.CancelSettingEqualCheck == FALSE )
	{
		return 0 ;
	}
	...
	GD3D11.Device.State.VerticalFogMode = Mode ;
...
extern int  Graphics_D3D11_DeviceState_SetVerticalFogColor( unsigned int Color )
{
	...
	if( Color == GD3D11.Device.State.VerticalFogColor &&
		GD3D11.Device.DrawSetting.CancelSettingEqualCheck == FALSE )
	{
		return 0 ;
	}
...
static int Graphics_D3D11_DeviceState_UpdateConstantVerticalFogParam( void )
{
	...
	{
		ConstantCommon->VerticalFog.LinearAdd    = -GD3D11.Device.State.VerticalFogEnd / ( GD3D11.Device.State.VerticalFogStart - GD3D11.Device.State.VerticalFogEnd ) ;
		ConstantCommon->VerticalFog.LinearDiv    = 1.0f / ( GD3D11.Device.State.VerticalFogStart - GD3D11.Device.State.VerticalFogEnd ) ;
	}
	else
	{
		ConstantCommon->VerticalFog.LinearAdd    = GD3D11.Device.State.VerticalFogEnd / ( GD3D11.Device.State.VerticalFogEnd - GD3D11.Device.State.VerticalFogStart ) ;
		ConstantCommon->VerticalFog.LinearDiv    = -1.0f / ( GD3D11.Device.State.VerticalFogEnd - GD3D11.Device.State.VerticalFogStart ) ;
	}
	ConstantCommon->VerticalFog.Density      = GD3D11.Device.State.VerticalFogDensity ;
	ConstantCommon->VerticalFog.DensityStart = GD3D11.Device.State.VerticalFogDensityStart ;
...
extern int  Graphics_D3D11_DeviceState_SetVerticalFogStartEnd( float Start, float End )
{
	...
	if( Start != GD3D11.Device.State.VerticalFogStart ||
		GD3D11.Device.DrawSetting.CancelSettingEqualCheck == TRUE )
	{
		GD3D11.Device.State.VerticalFogStart = Start ;
		UpdateFlag = TRUE ;
	}

	if( End != GD3D11.Device.State.VerticalFogEnd ||
		GD3D11.Device.DrawSetting.CancelSettingEqualCheck == TRUE )
	{
		GD3D11.Device.State.VerticalFogEnd = End ;
		UpdateFlag = TRUE ;
	}
...
extern int  Graphics_D3D11_DeviceState_SetVerticalFogDensity( float Start, float Density )
{
	...
	if( Density == GD3D11.Device.State.VerticalFogDensity &&
		GD3D11.Device.DrawSetting.CancelSettingEqualCheck == FALSE )
	{
		return 0 ;
	}
...
// ライティング計算でハーフランバートを使用するかどうかを設定する
extern int Graphics_D3D11_DrawSetting_SetLightUseHalfLambert( int UseHalfLambert )
{
	...
	if( GD3D11.Device.DrawSetting.UseHalfLambert == UseHalfLambert &&
		GD3D11.Device.DrawSetting.CancelSettingEqualCheck == FALSE )
	{
		return 0 ;
	}
...
// 高さフォグモードを設定する
extern	int		Graphics_Hardware_D3D11_SetVerticalFogMode_PF( int Mode /* DX_FOGMODE_NONE 等 */ )
{
...
extern	int		Graphics_Hardware_D3D11_ApplyLigFogToHardware_PF( void )
{
	...
	if( GD3D11.Device.State.VerticalFogEnable != GSYS.DrawSetting.VerticalFogEnable )
	{
...
extern int NS_SetUseDirect3D11AdapterLUID( LUID *UseLUID )
{
	...
	// 指定の LUID のアダプタが無ければエラー
	if( i == GD3D11.Adapter.InfoNum )
	{
		return -1 ;
	}

	// 指定の LUID のアダプタがあった場合は、そのアダプタを使用するように設定する
...
{
	// 使用する LUID が指定されていない場合は無効な値を返す
	if( GD3D11.Setting.EnableUseDXGIAdapterIndex == FALSE )
	{
		LUID NoneLUID ;
		_MEMSET( &NoneLUID, 0, sizeof( LUID ) ) ;
		return NoneLUID ;
	}
...
//	return DXGISwapChain_Present() ;
	...
	// ウインドウのクライアント領域のサイズがバックバッファーのサイズと異なる場合はバックバッファのサイズを変更する
	{
		...
		if( GD3D11.Device.Screen.TargetOutputWindow == 0 && WinData.UserWindowFlag == FALSE )
		{
