...
struct WINDATA
{
	...
	int						DisableWindows10_WM_CHAR_CancelTime ;	// Windows10 で WM_CHAR で短時間に連続して同じ文字が入力された場合の無効扱いにする処理を無効にするかどうかフラグ( TRUE：無効にする  FALSE：無効にしない )
...
//	int						DefaultRefreshRate ;				// 起動時のデスクトップのリフレッシュレート
	...
	MSGFUNC					MessageHookCallBack ;				// メッセージフックのコールバック関数
